...
function calcCase(c, vehicles, settings) {
  ...
  const tax = roundBy(taxable * (settings.taxRate / 100), settings.rounding);        // 税率ごとに1回の端数処理
  return { segs, segTotal, totalDays, optLines, optTotal, nocAmt, nocTaxable, other, discount, taxable, nonTaxable, subtotal: taxable, tax, total: taxable + tax + nonTaxable };
}
...
function CaseForm({ draft, vehicles, cases, settings, onSave, onCancel, onDelete }) {
  ...
  // 保存前検証(監査F-01/F-04/F-06対応): すべてブロッカー。警告承認での回避は不可
  const validate = () => {
    const errors = [];
    const lenient = c.status === "下書き" || c.status === "キャンセル";
    if (!lenient) {
      if (!c.customer.name?.trim()) errors.push("【必須】借受人の氏名を入力してください。");
      if (c.type === "事故代車" && !c.insurance.company?.trim()) errors.push("【必須】保険会社名を入力してください。");
      if (!(c.segments || []).length) errors.push("【必須】配車区間を1件以上登録してください。");
    }
    (c.segments || []).forEach((s, i) => {
      const v = vehicles.find((x) => x.id === s.vehicleId);
      if (!s.vehicleId || !s.start || !s.end) {
        if (!lenient) errors.push(`【必須】区間${i + 1} の車両・配車日・返却日をすべて入力してください。`);
        return;
      }
      // 日時の逆転(F-01)
      const a = dtOf(s.start, s.startTime || "00:00"), b = dtOf(s.end, s.endTime || "00:00");
      if (!(b > a)) {
        errors.push(`【期間エラー】区間${i + 1}（${v?.regNo || ""}）の返却日時が配車日時以前になっています。保存できません。`);
        return;
      }
      // 車両状態(F-04): 稼働以外は割当不可
      if (v.state !== "稼働") errors.push(`【割当不可】${v.regNo} は「${v.state}」のため貸渡に割り当てできません。`);
      // 車検(F-04): 返却予定日まで有効でなければハードブロック(承認による回避不可)
      if (!v.shaken) errors.push(`【割当不可】${v.regNo} の車検満了日が未登録のため割り当てできません。`);
      else if (v.shaken < s.end) errors.push(`【車検】${v.regNo} の車検満了日（${wareki(v.shaken)}）が返却予定日より前です。貸渡期間全体で車検が有効な車両のみ割当できます。`);
      // 同一案件内の区間重複(F-06)
      (c.segments || []).forEach((s2, j) => {
        if (j <= i || s2.vehicleId !== s.vehicleId || !s2.start || !s2.end) return;
        if (overlap(s.start, s.end, s2.start, s2.end)) errors.push(`【重複】区間${i + 1}と区間${j + 1}で同一車両（${v.regNo}）の期間が重複しています。`);
      });
      // 他案件との二重予約(A03)
      for (const other of cases) {
        if (other.id === c.id || other.deleted || ["キャンセル", "完了"].includes(other.status)) continue;
        for (const os of other.segments || []) {
          if (os.vehicleId === s.vehicleId && os.start && os.end && overlap(s.start, s.end, os.start, os.end)) {
            errors.push(`【二重予約】${v.regNo} は案件 ${other.caseNo}（${jpDate(os.start)}〜${jpDate(os.end)}）と期間が重複しています。保存できません。`);
          }
        }
      }
    });
  ...
  const trySave = () => {
    const errs = validate();
    if (errs.length) { setDialog({ title: "保存できません", msgs: errs, mode: "block" }); return; }
  ...
  const changeStatus = (st) => {
    ...
    // F-05: 貸渡開始条件(免許・本人確認・車両適格)をすべて満たすこと
    if (st === "貸渡中") {
      const msgs = [];
      const latestEnd = (c.segments || []).reduce((m, s) => (s.end && s.end > m ? s.end : m), "");
      if (!/^\d{12}$/.test(c.customer.licenseNo || "")) msgs.push("免許証番号（数字12桁）が未入力または桁数不正です。");
      if (!c.customer.licenseType?.trim()) msgs.push("免許の種類が未入力です。");
      if (!c.customer.licenseExpiry) msgs.push("免許有効期限が未入力です。");
      else if (latestEnd && c.customer.licenseExpiry < latestEnd) msgs.push(`免許有効期限（${jpDate(c.customer.licenseExpiry)}）が返却予定日（${jpDate(latestEnd)}）より前です。貸渡できません。`);
      else if (c.customer.licenseExpiry < todayStr()) msgs.push("免許証の有効期限が切れています。貸渡できません。");
      if (!c.customer.confirmed) msgs.push("担当者による本人確認（原本照合）が未完了です。");
      if (!c.customer.confirmedBy?.trim()) msgs.push("本人確認の確認者名が未入力です。");
      const vErrs = validate();
      msgs.push(...vErrs);
      if (msgs.length) { setDialog({ title: "貸渡を開始できません", msgs, mode: "block" }); return; }
    }
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 space-y-5">
        ...
        <Card title="借受人・本人確認" action={
          ...
          <label className="mt-3 flex items-center gap-2 text-sm font-medium bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 cursor-pointer">
            ...
            原本（免許証）と照合し、担当者が内容を確認しました（貸渡開始の必須条件）
          </label>
        </Card>
        ...
        <Card title="料金計算（自動）">
          <table className="w-full text-sm">
            <tbody>
              ...
              <tr><td className="py-1.5">消費税（{settings.taxRate}%・{settings.rounding === "round" ? "四捨五入" : settings.rounding === "floor" ? "切捨て" : "切上げ"}・税率ごとに1回）</td><td className="py-1.5 text-right">{yen(calc.tax)}</td></tr>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-5">
            ...
            <div className="mt-4 flex gap-2 justify-end">
              ...
              {dialog.mode === "confirm" && (
                <button onClick={() => { setDialog(null); onSave(c); }} className="px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-sm font-semibold">承認して保存</button>
              )}
              {dialog.mode === "delete" && (
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
        <table style={{ width: "48%" }}>
          <tbody>
            ...
            <tr><th>消費税（{settings.taxRate}%）</th><td style={{ textAlign: "right" }}>{yen(calc.tax)}</td></tr>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, gap: 16 }}>
        ...
        <table style={{ width: "44%" }}>
          <tbody>
            ...
            <tr><th>消費税（{ds.taxRate}%）</th><td style={{ textAlign: "right" }}>{yen(dc.tax)}</td></tr>
