...
function CaseForm({ draft, vehicles, cases, settings, onSave, onCancel, onDelete }) {
  ...
  const changeStatus = (st) => {
    ...
    // 貸渡開始時は免許・本人確認を点検(未充足でも確認のうえ開始可能)
    if (st === "貸渡中") {
      const msgs = [];
      const latestEnd = (c.segments || []).reduce((m, s2) => (s2.end && s2.end > m ? s2.end : m), "");
      ...
      else if (c.customer.licenseExpiry < todayStr()) msgs.push("免許証の有効期限が切れています。無免許運転となるため貸渡は行わないでください。");
      else if (latestEnd && c.customer.licenseExpiry < latestEnd) msgs.push(`免許有効期限（${jpDate(c.customer.licenseExpiry)}）が返却予定日（${jpDate(latestEnd)}）より前です。`);
      if (!c.customer.confirmed) msgs.push("担当者による本人確認（原本照合）が未完了です。");
      if (!c.customer.confirmedBy?.trim()) msgs.push("本人確認の確認者名が未入力です。");
      msgs.push(...warns);
      if (msgs.length) { setDialog({ title: "確認事項があります（貸渡開始）", msgs, mode: "confirm-status", payload: st }); return; }
      applyStatus(st);
      return;
    }
